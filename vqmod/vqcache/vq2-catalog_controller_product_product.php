			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                ...
                if($this->config->get('catalogrules_status') == 1 && ($this->config->get('catalogrules_date_start') <= $date_now) && ($this->config->get('catalogrules_date_end') >= $date_now))
				{
                  ...
                  if($discoutType == 'P')
                  {
                    ...
                    if($product_info['tax_class_id'] != 0 && $this->config->get('catalogrules_discount_tax') == 1)
                    {
	                  $Taxprice = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
	                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
	                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
	                  $data['price'] =  '<B>In Tax Discount </B>'.$this->currency->format($afterDiscount);

                     }else
                     {

                      $Taxprice = $this->tax->calculate($product_info['price'], 0 , $this->config->get('config_tax'));
	                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
	                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
	                  $data['price'] =  '<B>Ex. Tax Discount </B>'.$this->currency->format($afterDiscount);
			...
			if ((float)$product_info['special']) {
                ...
                if($this->config->get('catalogrules_status') == 1 && ($this->config->get('catalogrules_date_start') <= $date_now) && ($this->config->get('catalogrules_date_end') >= $date_now))
				{
                  ...
                  if($discoutType == 'P')
                  {
                    ...
                    if($product_info['tax_class_id'] != 0 && $this->config->get('catalogrules_discount_tax') == 1)
                    {
	                  $Taxprice = $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'));
	                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
	                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
	                  $data['special'] =  '<B>In Tax Discount </B>'.$this->currency->format($afterDiscount);

                     }else
                     {

                      $Taxprice = $this->tax->calculate($product_info['special'], 0 , $this->config->get('config_tax'));
	                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
	                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
	                  $data['special'] =  '<B>Ex. Tax Discount </B>'.$this->currency->format($afterDiscount);
