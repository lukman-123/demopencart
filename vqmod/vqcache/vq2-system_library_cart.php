   	       	   ...
   	       	   if($this->config->get('catalogrules_status') == 1 && ($this->config->get('catalogrules_date_start') <= $date_now) && ($this->config->get('catalogrules_date_end') >= $date_now))
				{
					...
					$discoutType = $this->config->get('catalogrules_type');
					  ...
					  if($discoutType == 'P')
	                  {
		                ...
		                if($product_query->row['tax_class_id'] != 0 && $this->config->get('catalogrules_discount_tax') == 1)
	                    {
	                      $Taxprice = $this->tax->calculate($product_query->row['price'], $product_query->row['tax_class_id'], $this->config->get('config_tax'));
		                 ...
		                 {
		                  $Taxprice = $this->tax->calculate($product_query->row['price'], 0, $this->config->get('config_tax'));
		                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
		                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
		                  $price = $afterDiscount;		                 
		                 } 
			   ...
			   $date_now = date("Y-m-d");
   	       	   if($this->config->get('catalogrules_status') == 1 && ($this->config->get('catalogrules_date_start') <= $date_now) && ($this->config->get('catalogrules_date_end') >= $date_now))
				{
					...
					$discoutType = $this->config->get('catalogrules_type');
					  ...
					  if($discoutType == 'P')
	                  {
		                ...
		                if($product_query->row['tax_class_id'] != 0 && $this->config->get('catalogrules_discount_tax') == 1)
	                    {
	                      $Taxprice = $this->tax->calculate($product_discount_query->row['price'], $product_query->row['tax_class_id'], $this->config->get('config_tax'));
		                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
		                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
		                  $price = $afterDiscount;
		                 }
		                 ...
		                 {
		                  $Taxprice = $this->tax->calculate($product_discount_query->row['price'], 0, $this->config->get('config_tax'));
		                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
		                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
		                  $price = $afterDiscount;		                 
		                 } 
   	       	   ...
   	       	   if($this->config->get('catalogrules_status') == 1 && ($this->config->get('catalogrules_date_start') <= $date_now) && ($this->config->get('catalogrules_date_end') >= $date_now))
				{
					...
					$discoutType = $this->config->get('catalogrules_type');
					  ...
					  if($discoutType == 'P')
	                  {
		                ...
		                if($product_query->row['tax_class_id'] != 0 && $this->config->get('catalogrules_discount_tax') == 1)
	                    {
	                      $Taxprice = $this->tax->calculate($product_special_query->row['price'], $product_query->row['tax_class_id'], $this->config->get('config_tax'));
		                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
		                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
		                  $price = $afterDiscount;
		                 }
		                 ...
		                 {
		                  $Taxprice = $this->tax->calculate($product_special_query->row['price'], 0, $this->config->get('config_tax'));
		                  $afterDiscountPercentage = $Taxprice * $discoutPrice/100;
		                  $afterDiscount = $Taxprice - $afterDiscountPercentage;
		                  $price = $afterDiscount;		                 
		                 } 
				...
				if ($recurring_query->num_rows) {
					...
					$recurring = false;
               	    ...
               	    if($this->config->get('catalogrules_status') == 1)
					{
                      $tax_class_id = $this->config->get('catalogrules_discount_tax') ? 0 :  $product_query->row['tax_class_id'];
					}else
					{
